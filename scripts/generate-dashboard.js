...
function renderSVG(data) {
  ...
  const heroStatsSVG = statRows
    .map((s, i) => {
      ...
      <g transform="translate(${HERO_X + 22}, ${y})">
        ...
        <text x="${HERO_W - 44}" y="0" text-anchor="end" class="hero-stat-value">${s.value.toLocaleString()}</text>
      </g>`;
    })
    .join("");
  ...
  const langBarsSVG = data.topLanguages
    .map((lang, i) => {
      ...
      <g transform="translate(${RIGHT_X + 22}, ${y})">
        ...
        <rect x="${barX}" y="-9" width="${maxBarWidth}" height="7" rx="3.5" fill="#171b22"/>
        <rect x="${barX}" y="-9" width="${barWidth}" height="7" rx="3.5" fill="${lang.color}" fill-opacity="0.85"/>
      </g>`;
    })
    .join("");
  ...
  const repoRowsSVG = data.topRepos
    .map((repo, i) => {
      ...
      return `
      <g transform="translate(${RIGHT_X + 22}, ${y})">
        <circle cx="4" cy="-5" r="4" fill="${repo.color}"/>
  ...
  <defs>
    ...
    <linearGradient id="ringGrad" gradientUnits="userSpaceOnUse" x1="${avatarCx - avatarR - 6}" y1="${avatarCy - avatarR - 6}" x2="${avatarCx + avatarR + 6}" y2="${avatarCy + avatarR + 6}">
      <stop offset="0%" stop-color="#7c9cff"/>
      <stop offset="50%" stop-color="#4b6bd6"/>
      <stop offset="100%" stop-color="#7c9cff"/>
      <animateTransform attributeName="gradientTransform" type="rotate"
        from="0 ${avatarCx} ${avatarCy}" to="360 ${avatarCx} ${avatarCy}" dur="10s" repeatCount="indefinite"/>
    </linearGradient>
    <radialGradient id="blobIndigo" cx="50%" cy="50%" r="50%">
      <stop offset="0%" stop-color="#3b4a8f" stop-opacity="0.22"/>
      <stop offset="100%" stop-color="#3b4a8f" stop-opacity="0"/>
    </radialGradient>
    <radialGradient id="blobCyan" cx="50%" cy="50%" r="50%">
      <stop offset="0%" stop-color="#2a5566" stop-opacity="0.18"/>
      <stop offset="100%" stop-color="#2a5566" stop-opacity="0"/>
    </radialGradient>
  ...
  <style>
    ...
    .hero-stat-value { fill: #d7e0f5; font-size: 19px; font-weight: 700; font-family: ui-monospace, "SF Mono", Menlo, Consolas, monospace; }
    ...
    .repo-stars { fill: #c9a24a; font-size: 12.5px; font-weight: 600; font-family: ui-monospace, "SF Mono", Menlo, Consolas, monospace; }
    .footer { fill: #4b5563; font-size: 11px; }
    .glass { fill: #ffffff; fill-opacity: 0.02; stroke: #ffffff; stroke-opacity: 0.06; stroke-width: 1; }

    @keyframes drift1 { 0%, 100% { transform: translate(0,0); } 50% { transform: translate(24px, 14px); } }
    @keyframes drift2 { 0%, 100% { transform: translate(0,0); } 50% { transform: translate(-20px, -12px); } }

    .blob-a { animation: drift1 16s ease-in-out infinite; }
  ...
  <rect x="${HERO_X}" y="${HERO_Y}" width="${HERO_W}" height="${HERO_H}" rx="18" fill="none" stroke="#3b4454" stroke-width="1"/>
  ...
  <rect x="${RIGHT_X}" y="${LANG_Y}" width="${RIGHT_W}" height="${LANG_H}" rx="18" fill="none" stroke="#3b4454" stroke-width="1"/>
  ...
  <rect x="${RIGHT_X}" y="${REPO_Y}" width="${RIGHT_W}" height="${REPO_H}" rx="18" fill="none" stroke="#3b4454" stroke-width="1"/>
